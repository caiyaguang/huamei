(function(g) {
	...
	g.browser.name = (f.match(/\b(chrome|opera|safari|msie|firefox)\b/) || ["", "mozilla"])[1];
	g.browser.core = (f.match(/msie|webkit|gecko|opera|khtml/) || "msie");
	g.browser.ver = g.browser.version = (f.match(/.(?:rv|it|ra|ie)[\/: ]([\d.]+)/) || [0, "0"])[1];
	if (window.Base64) {
		return
	} else {
		function e(m) {
	...
	g.extend({
		getScript: function(l, k) {
			...
			if (arguments.length > 1) {
				...
				case "object":
					...
					m = g.type(k.crossDomain) == "boolean" ? k.crossDomain : m;
					n = g.type(k.callback) == "function" ? k.callback : n;
					o = g.type(k.parameter) == "array" ? k.parameter : o;
					break
				}
			}
			g.ajax({
		...
		getscript: function(m, n) {
			var n = g.extend({}, n);
			n._d = n._d || document;
			var k = n._d.getElementsByTagName("head")[0],
				l = n._d.createElement("script"),
				j = n.arg || [];
			...
			if (g.browser.msie) {
				l.onreadystatechange = function() {
					var o = l.readyState;
					if (o == "loaded" || o == "interactive" || o == "complete") {
		...
		cookie: function(m, n, k) {
			...
			if (arguments.length > 1 && String(n) !== "[object Object]") {
				...
				n = String(n);
				return (document.cookie = [encodeURIComponent(m), "=", k.raw ? n : encodeURIComponent(n), k.expires ? "; expires=" + k.expires.toUTCString() : "", k.path ? "; path=" + k.path : "", k.domain ? "; domain=" + k.domain : "", k.secure ? "; secure" : ""].join(""))
			}
			k = n || {};
			var j, o = k.raw ?
			function(r) {
			...
			return (j = new RegExp("(?:^|; )" + encodeURIComponent(m) + "=([^;]*)").exec(document.cookie)) ? o(j[1]) : ""
		},
		getParm: function(l, r) {
			var q, p = window.location.toString(),
				k = l ? l : p;
			if (g.type(k) == "object") {
				q = g.param(k);
				p = r ? r : p;
				...
				if (g.type(k) == "string") {
					...
					if (m && m.indexOf("&")) {
						var n = m.split("&");
						jQuery.each(n, function(o, t) {
							if (t && t.indexOf("=")) {
								var s = t.split("=");
								if (g.type(r) == "string" && r == s[0]) {
									q = s[1] == null ? "" : s[1];
									return true
								} else {
									q[s[0]] = s[1]
		...
		getUrl: function(k) {
			var k = k || window.location.toString(),
				l = /(\w+):\/\/([^\:|\/]+)(\:\d*)?(.*\/)([^#|\?|\n]+)?(#.*)?(\?.*)?/i;
			var j = k.match(l);
			if ( !! !j) {
				var l = k.match(/(\w+):\/\/(.*)/);
		...
		getBrowserType: function() {
			...
			return g.browser.name == "msie" ? j.msie["v" + parseInt(g.browser.ver)] : j[g.browser.name]
		},
...
var PassportSC = {
	...
	loginProtocal: "https",
	loginURL: "PASSPORT.TUAN800.COM/sso/login",
	logoutURL: "PASSPORT.TUAN800.COM/sso/logout",
	vurl: "https://PASSPORT.TUAN800.COM/sso/scripts?type=",
	http_url: false,
	login_urltpl: "{{#http}}://{{#url}}?username={{#username}}&password={{#pwd_md5}}&appid={{#appid}}&rememberme={{#pc}}&s={{#s}}&b={{#b}}&w={{#w}}&pwdtype=1&v={{#version}}",
	logins_urltpl: "{{#http}}://{{#url}}?username={{#username}}&password={{#pwd_md5}}&appid={{#appid}}&rememberme={{#pc}}&s={{#s}}&b={{#b}}&w={{#w}}&pwdtype=2&v={{#version}}",
	...
	domainList: ["keyinfo", "163.com", "qq.com", "hotmail.com", "gmail.com", "139.cn", "189.cn"],
	appList: {
		"1000": "tuan800",
		"2000": "zhe800",
		"3000": "hui800"
	...
	reportMsg: function(c, a, b) {
		...
		case "11":
			...
			break;
		case "12":
			d += "账号尚未激活，重发<a href=" + this.loginProtocal + '"://passport.tuan800.com/users/confirmation/new?email=' + this.usernameInput.val() + '" target="_blank">激活邮件</a>';
			break;
		case "error":
			d = a.message;
			break;
	...
	doPost: function() {
		...
		if (e == document.forms.length) {
			document.location.href = this.loginProtocal + "://passport.tuan800.com";
			return false
		}
		var d = {
		...
		var c = screen.width;
		document.forms[e].action = $.ParseTpl("{{#url}}?appid={{#appid}}&ru={{#ru}}&b={{#b}}&w={{#w}}&v={{#version}}", d);
	...
	doLogin: function() {
		...
		return this.loginHandle(e, c, a, this.sElement, this.loginFailCall.bindFunc(this), this.loginSuccessCall.bindFunc(this))
	},
	loginHandle: function(g, k, m, r, l, c) {
		...
		if (this.captchaInput && this.captchaInput.val() != "") {
			a += "&captcha=" + this.captchaInput.val() + "&captcha_key=" + login_error_result.captcha_key
		}
		login_status = "";
		login_error_result = "";
		login_need_captcha = false;
		...
		if (this.intervalCount >= this.maxIntervalCount) {
			...
			if ($.cookie() == false) {
				...
				if (login_need_captcha && this.captchaInput) {
					...
					this.captchaInput.closest(".token").show().find("img").click(function() {
						c($(this))
					})
				}
				this.passwdInput.focus();
	...
	refreshCookie: function(e, a, d) {
		var b = this.loginProtocal + "://passport.tuan800.com/sso/refresh?_t=" + new Date().getTime();
		if (b) {
			var c = $("<iframe></iframe>");
			c.css({
				height: 0,
	...
	renewCookie: function(f, b, e) {
		...
		var g = this.getTime();
		var c = this.loginProtocal + "://passport.tuan800.com/sso/refresh?ra" + g;
		c += "&domain=" + this.domain;
		if (c) {
			var d = $("<iframe></iframe>");
			d.css({
	...
	doCrossDomainCookie: function(p, h) {
		...
		var g = function() {
				...
				q.css({
					height: 0,
					width: 0
				}).attr("src", n.loginProtocal + "://passport.tuan800.com/sso/refresh?_t" + new Date().getTime());
				p.append(q)
			};
		if (a == "" || a == "0") {
			if (b == "" && h == "logout" && e != "") {
				...
				if (b != "" && h == "login") {
					...
					if (e == "") {
						...
						f.setHours(0, 0, 0);
						d.setHours(0, 0, 0);
						var o = Math.floor((Date.parse(f) - Date.parse(d)) / 24 / 1000 / 3600);
						if (o > 0) {
							g();
	...
	crossDomainUrl: function(c, b) {
		var d = this.getTime();
		var a = this.loginProtocal + "://passport.tuan800.com/sso/scripts?s=" + d + "&type=" + c + "&current_site=" + b;
		return a
	},
	...
	upDSindex: function() {
		...
		if (b == a.size()) {
			...
			if (d == -1) {
				this.curDSindex = a.eq(a.size() - 1).find("td:first").prop("idx")
			} else {
				this.curDSindex = d
			}
		}
	...
	fillEmailSelect: function() {
		...
		var k = parseInt(this.usernameInput.offset().left - this.cElement.offset().left);
		if ($.browser.msie) {}
		this.dsElement.css("margin-left", k + "px");
		this.dsElement.css("margin-top", this.usernameInput.offset().top - this.cElement.offset().top + this.usernameInput.get(0).offsetHeight - 1 + "px");
		this.dsElement.css({
			"z-index": 999999,
			padding: 0,
			"background-color": "white"
		});
		this.dsAnchor.empty().append('<table width="100%" cellspaceing="0" cellpadding="3"><tbody></tbody></table>');
	...
	doSelect: function() {
		...
		if (a) {
			var b = a.html();
			if (b) {
				this.usernameInput.val(b.replace(/&amp;/g, "&"))
			}
		}
	...
	rootElement: function(b) {
		this.rootElement = b;
		this.rootElement.html('<div class="ppselecter" style="position: absolute; display: none;"><table width="100%" cellspacing="0" cellpadding="0"><tbody><tr><td style="" class="ppseltit">' + this.selectorTitle + '</td></tr><tr><td /></tr></tbody></table></div><div style="display: none;"></div><div class="passportc"></div>');
		if (this.selectorTitle == null || this.selectorTitle.length == 0) {
			this.rootElement.html('<div class="ppselecter" style="position: absolute; display: none;"><table width="100%" cellspacing="0" cellpadding="0"><tbody><tr></tr><tr><td height="0" /></tr><tr><td /></tr></tbody></table></div><div style="display: none;"></div><div class="passportc"></div>')
		}
	...
	bindSelector: function() {
		if (this.bindDomainSelector) {
			this.curDSindex = -1;
			this.usernameInput.bind("mousedown", this.checkMousedown.bindFunc(this)).bind("keypress", this.checkKeyPress.bindFunc(this)).bind("keyup", this.checkKeyUp.bindFunc(this)).bind("blur", this.doSelect.bindFunc(this));
			if ($.browser.msie) {
				this.usernameInput.bind("keydown", this.checkKeyDown.bindFunc(this))
			}
		}
	},
	drawPassport: function(element) {
		...
		if (this.md5pwd) {
			$.getScript("https://passport.tuan800.com/js/jquery.md5.min.js", true)
		}
	...
	drawPassportNew: function(c, j, h) {
		...
		d.successCalledFunc = (a === "function") ? a : ($.type(h) === "object") ? h.sufun ? h.sufun : false : false;
		d.appid = ($.type(j) === "number") ? j : ($.type(h) === "object") ? h.appid ? h.appid : 9999 : 9999;
		d.curCardIndex = b;
		d.isSetFocus = false;
