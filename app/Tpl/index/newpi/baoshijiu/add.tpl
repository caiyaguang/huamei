...
<meta name="copyright" content="2010-2014 huameitaoke Inc." />
...
<div class="main {:C('ftx_site_width')} mb20">
	<div class="main-newgood -2" style="text-align:left">
		...
		<form action="{:U('baoshijiu/add',array('do'=>'submit'))}" method="post" name="form" onsubmit="return check_zhe();">
			<div class="bm-form">
				...
				<table>
					...
					<option value="16">20元封顶</option></select>					
					</td>
					</tr>
					<tr>
						<th>包邮范围：</th>
...
function getTaoItem(url){
	...
	$.getJSON(FTXIAER.root + '{:U("baoshijiu/ajaxGetItem")}', {url:url}, function(result){
...
$("#smt").click(function() {
	...
	$.ajax({
	url: FTXIAER.root + '/index.php?m=baoshijiu&a=ajaxadd',
	type: 'POST',
	data: {
	iid:iid,	
	nick:nick,
	...
	if(result.status == 1){
			    $.dialog({id:'goods_shijiu_success', title:lang.tips, content:result.msg['purl']+result.data, padding:'', fixed:true, lock:true});		
                        }else{
...
$("#smt").click(function() {
	...
	$.ajax({
            ...
            success: function(result){
            if(result.status == 1){
			    $.dialog({id:'goods_baoshijiu_success', title:lang.tips, content:result.data, padding:'', fixed:true, lock:true});
                        }else{
                            $.ftxia.tip({content:result.msg, icon:'error'});
                        }
                    }
