...
<form method="post" action="{:U('user/shensu')}" name="frm"	enctype="multipart/form-data">
<div style="border-radius: 4px; border: 1px solid #BCE3FE; background: #F0F9FF; padding: 10px 5px 5px; margin: 10px;">
<table>
	<tr>
		<td>申诉理由：</td>
	...
	<tr>
		...
		<td><input type="file" name="ss_pz2" /> 
		<input type="hidden" id="order_id" name="order_id" value="" />
		</td>
	</tr>
...
<div style="margin:10px 10px 10px 0px;">
			...
			<tr>
			 ...
			 <td align="center" class="td_border" style="padding:5px;width:100px;"><a target="_blank" href="{$val.items_id|getTableValue='id','free','click_url'}"><div style="width:98px;height:30px;line-height:30px;overflow:hidden">{$val.title}</div><img width="90" src="{$val.items_id|getTableValue='id','free','pic_url'}"></a></td>
					  ...
					  <td align="center" class="td_border">{$val.order_id}<br/>
					    ...
					    <td align="center" class="td_border">{$val.status}</td>
						 <td class="td_borders" align="center" style="width:100px;padding:5px">  
                           <if condition="$val.status eq '待填写订单号'">
						   <a class="utai" href="{$val.items_id|getTableValue='id','free','click_url'}" target="_blank">前往下单</a>
						   <a class="utai" href="javascript:void(0)" onclick="tiandanhao({$val.id})">提交单号</a>
						   <elseif condition="$val.status eq '待审核订单号'"/>
						   <span class="utai" href="">待返现</span>
						   <a class="utai" href="javascript:void(0)" onclick="tiandanhao({$val.id})">修改单号</a>
						   <elseif condition="$val.status eq '订单号有误'"/>
						   <a class="utai" href="javascript:void(0)" onclick="complain({$val.id})">提交申诉</a>
						   <a class="utai" href="javascript:void(0)" onclick="tiandanhao({$val.id})">修改单号</a>
