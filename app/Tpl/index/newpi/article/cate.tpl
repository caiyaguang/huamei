...
<div class="wrap_960">
	...
	<div class="tip">
    您当前所在位置：<a href="{:C('ftx_site_url')}">网站首页</a>&gt; <a href="{:U('article/index')}">文章中心</a>&gt; <a href="{:U('article/cate',array('id'=>$cat['id']))}">{$cat.name}</a>
    </div>  
...
with(document) 0[(getElementsByTagName('head')[0] || body).appendChild(createElement('script')).src = 'http://bdimg.share.baidu.com/static/api/js/share.js?v=89860593.js?cdnversion=' + ~ (-new Date() / 36e5)]; </script>
		...
		<if condition="$cat.cateimg eq ''"><img src="{:C('ftx_site_flogo')}" alt="{$cat.name}" /><else /><img src="{$cat.cateimg}" alt="{$cat.name}"/></if>
        <div class="top-infos"><h1>{$cat.name}</h1>
		<p>
        {$cat.seo_desc}
        </p>
        </div>
