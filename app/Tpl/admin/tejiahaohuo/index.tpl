...
<div class="subnav">
  <h1 class="title_2 line_x">阿里妈妈频道采集器-特价好货<font color="red">[新]</font></h1>
</div>
<div class="pad_lr_10" >
  <form id="J_info_form" action="{:U('tejiahaohuo/setting')}" method="post" >
    <table width="100%" cellspacing="0" class="table_form">
      <tbody>
      
      	<tr>
          <th width="150">特价好货分类 :</th>
          <td>
             <select name="cat">
        ...
        <tr>
          <th width="150">佣金比率 :</th>
          <td>
            <input type="text" name="startTkRate" class="input-text" size="5" value="" /> - 
            <input type="text" name="endTkRate" class="input-text" size="5" value="" />
...
<script>
$(function(){
    var collect_url = "{:U('tejiahaohuo/collect')}";
    $('#J_info_form').ajaxForm({success:complete, dataType:'json'});
    var p = 2;
    function complete(result){
        if(result.status == 1){
            $.dialog({id:'tejiahaohuo', title:result.msg, content:result.data, padding:'', lock:true});
            p = 2;
            collect_page();
        } else {
    ...
    function collect_page(){
        $.getJSON(collect_url, {p:p}, function(result){
            if(result.status == 1){
                $.dialog.get('tejiahaohuo').content(result.data);
                p++;
                collect_page(p);
            }else{
                $.dialog.get('tejiahaohuo').close();
