...
<div class="pad_10" >
 ...
 <div class="J_tablelist table_list" data-acturi="{:U('items/ajax_edit')}">
    <table width="100%" cellspacing="0">
		...
		<th width="30"><span data-tdtype="order_by" data-field="inventory">库存</span></th>
		<th width="70"><span data-tdtype="order_by" data-field="realname">真实姓名</span></th>
		<th width="70"><span data-tdtype="order_by" data-field="mobile">联系电话</span></th>
		<th width="70"><span data-tdtype="order_by" data-field="qq">QQ</span></th>
    	...
    	<tbody>
			...
			<notempty name="val['pic_urls']">
				<div class="img_border"><a href="http://detail.tmall.com/item.htm?id={$val.num_iid}" target="_blank"><img src="{:attach($val['pic_urls'], 'item')}" width="32" width="32" class="J_preview" data-bimg="{:attach($val['pic_url'], 'item')}"></a></div>
			<else />
		...
		<td align="center" class="red">{$val.realname}</td> 
		<td align="center" class="red">{$val.mobile}</td> 
		<td align="center" class="red">{$val.qq}</td> 
                ...
                <td align="center"><a href="{:u('items/edit', array('id'=>$val['id'], 'menuid'=>$menuid))}">{:L('edit')}</a> | <a href="javascript:void(0);" class="J_confirmurl" data-uri="{:u('items/delete', array('id'=>$val['id']))}" data-acttype="ajax" data-msg="{:sprintf(L('confirm_delete_one'),$val['title'])}">{:L('delete')}</a> | <a class="showdialog" href="javascript:check({$val.id});" data-uri="{:U('items/getcheck',array('id'=>$val['id']))}" data-title="审核管理" data-id="add" data-width="500" data-height="180">审核意见</a></td>
            </tr>
            </volist>
    	</tbody>
    </table>
    ...
    <div class="btn_wrap_fixed">
		...
		<input type="button" class="btn btn_submit" data-tdtype="batch_action" data-acttype="ajax" data-uri="{:U('items/f_check')}" data-name="id" data-msg="你确定要全部初审通过吗？" value="初审" />
