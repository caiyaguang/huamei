...
<div class="subnav">
    <h1 class="title_2 line_x">九块邮-U站采集</h1>
</div>

<div class="pad_lr_10">
    <form id="J_info_form" action="{:U('uzjiu/setting')}" method="post">
    <table width="100%" cellspacing="0" class="table_form">
		<tr>
            <th width="150">选择九块邮U站分类 :</th>
            <td>
            <select name="uzjiu_cate_id">
	...
	<br><br>
	<h2>友情提示：本插件是直接采集九块邮官方U站的商品，同步获取卖家旺旺和卖家数字ID。采集过程中会自动过虑已存在的商品和信息不完整的商品，所以运行过程时间会长一点，视个人网站服务器配置情况而定，如果响应时间过长，请刷新后重新采集，或重启服务器重新采集，建议选择网络不繁忙的时段进行采集。</h2>
</div>
<include file="public:footer" />
<script>
$(function(){
    var collect_url = "{:U('uzjiu/collect')}";
    $('#J_info_form').ajaxForm({success:complete, dataType:'json'});
    var p = 2;
    function complete(result){
        if(result.status == 1){
            $.dialog({id:'uzjiu', title:result.msg, content:result.data, padding:'', lock:true});
            p = 2;
            collect_page();
        } else {
    ...
    function collect_page(){
        $.getJSON(collect_url, {p:p}, function(result){
            if(result.status == 1){
                $.dialog.get('uzjiu').content(result.data);
                p++;
                collect_page(p);
            }else{
                $.dialog.get('uzjiu').close();
