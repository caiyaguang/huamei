<?php
class baojiuAction extends FrontAction {
	...
	public function add(){
        ...
        if (C('ftx_site_haopinzhebao') == 1) {
        	...
        	$yanzbaotype  =  in_array('baojiu',$baotype ); //成功返回1 没有返回null
        	if ($yanzbaotype == 1) {
        		if ($info['signup'] <= 0) {
	...
	public function ajaxadd(){
		if(IS_POST){
			...
			if($items_mod->add()){	
			    ...
			    $resp = $this->fetch('dialog:goods_jiu_success');
			...
			if($items_mod->add()){
				...
				$resp = $this->fetch('dialog:goods_baojiu_success');
