...
class adAction extends BackendAction 
{
	...
	public function _search() 
	{
		...
		$this->assign('search', array( 'start_time_min' => $start_time_min, 'start_time_max' => $start_time_max, 'end_time_min' => $end_time_min, 'end_time_max' => $end_time_max, 'board_id' => $board_id, 'style' => $style, 'keyword' => $keyword, ));
		return $map;
	}
	public function _before_index() 
	{
		$big_menu = array( 'title' => L('ad_add'), 'iframe' => U('ad/add'), 'id' => 'add', 'width' => '520', 'height' => '350', );
		$this->assign('big_menu', $big_menu);
		$this->assign('img_dir', '/data/upload/banner/');
	...
	protected function _before_insert($data) 
	{
		...
		switch ($data['type']) 
		{
			case 'text': $data['content'] = $this->_post('text', 'trim');
			break;
			case 'image': $data['content'] = $this->_post('img', 'trim');
			break;
			case 'code': $data['content'] = $this->_post('code', 'trim');
			break;
			default : $this->ajaxReturn(0, L('ad_type_error'));
			break;
		}
		return $data;
	}
	...
	public function ajax_upload_img() 
	{
		...
		{
			$dir = date('ym/d/');
			$result = $this->_upload($_FILES[$type], 'banner/'. $dir );
			if ($result['error']) 
			{
