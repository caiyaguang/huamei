...
class advertWidget extends Action {
    ...
    public function index($id) {
		...
		$board_info['height']= 'height:'.$board_info['height'].'px';
		$adheight = $board_info['height'];
		$this->assign('adheight', $adheight);
    ...
    private function _get_html($ad, $board_info) {
        ...
        switch ($ad['type']) {
            case 'image':
                $html  = '<a title="'.$ad['name'].'" href="'.U('banner/tgo',array('id'=>$ad['id'])).'" target="_blank">';
                $html .= '<img alt="'.$ad['name'].'" src="'.__ROOT__.'/data/upload/banner/'.$ad['content'].'" '.$size_html.'>';
                $html .= '</a>';
                break;
        }
        return $html;
