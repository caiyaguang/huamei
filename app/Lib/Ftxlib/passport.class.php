...
class passport
{
     ...
     * 注册新用户
     */
    public function register($username, $password, $email, $gender, $login_type, $qq) {
        if (!$add_data = $this->_us->register($username, $password, $email, $qq, $gender, $login_type)) {
            $this->_error = $this->_us->get_error();
            return false;
        }
        //添加到本地
		$add_data['login_type'] = $login_type;
    ...
    private function _local_add($add_data) {
        $user_mod = D('user');
		if(!$add_data['login_type']){
			$add_data['login_type'] = '1';
		}
