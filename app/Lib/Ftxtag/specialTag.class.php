...
class specialTag {
	...
	public function lists($options) {
		...
		$options['num'] = isset($options['num']) ? trim($options['num']) : '10000';
		$options['order'] = isset($options['order']) ? trim($options['order']) : 'add_time desc';	
		if($options['cid']){			
		$id_arr = D('zc_cate')->get_child_ids($options['cid'], true);
		$map['cate_id'] = array('IN', $id_arr);
		}
		$map['pass'] = '1';
        $data = $zc_mod->where($map)->limit('0 ,' . $options['num'])->order($options['order'])->select();
        return $data;
    }
