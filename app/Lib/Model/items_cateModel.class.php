...
class items_cateModel extends Model
{
     ...
     * 根据ID获取分类图标 
     */
    public function get_cateimg($id) {
        //分类数据
        if (false === $cate_data = F('cate_data')) {
            $cate_data = $this->cate_data_cache();
        }
        return $cate_data[$id]['cateimg'];
    }
    ...
    public function cate_cache() {
		...
		if(C('ftx_site_cache')){
			if(false === $cate_list = S('cate_list')){
				$cate_data = $this->field('id,cateimg,pid,name')->where('status=1')->order('ordid')->select();
    ...
    public function cate_data_cache() {
		...
		if(C('ftx_site_cache')){
			if(false === $cate_data = S('cate_data')){
				$result = $this->field('id,cateimg,pid,spid,name,seo_title,seo_keys,seo_desc')->where('status=1')->order('ordid')->select();
    ...
    public function relate_cache() {
        $cate_relate = array();
        $cate_data = $this->field('id,cateimg,pid,spid')->where('status=1')->order('ordid')->select();
        foreach ($cate_data as $val) {
